document.addEventListener("DOMContentLoaded", () => {
  ...
  // Fade-in immagine overlay
  setTimeout(() => { countImg.style.opacity = "1"; }, 100);

  // Dopo 3 secondi: fade-out e rimozione overlay
  setTimeout(() => {
    countImg.style.opacity = "0";
    setTimeout(() => {
      overlay.style.display = "none"; 
      console.log("Overlay finito, parte AR puzzle!");
      initPuzzle(); // avvia il puzzle SOLO dopo overlay
    }, 1000);
  }, 3000);
    ...
    function shuffle(times=10) {
      for (let i=0;i<times;i++){
        ...
        if(neighbors.length>0){
          ...
          tryMove(piece);document.addEventListener("DOMContentLoaded", () => {
  const overlay = document.getElementById("countOverlay");
  const countImg = document.getElementById("countImg");
  ...
  // --- Funzione originale del puzzle ---
  function initPuzzle() {
    // Tutto il tuo codice originale che mi hai mandato rimane **invariato**
    const marker = document.getElementById("marker");
    const container = document.getElementById("piecesContainer");
    const cameraEl = document.querySelector("a-camera");
    ...
    function checkSolved() {
      ...
      if (solved) {
        console.log("Puzzle completato! 🎉");
        // ... resto della logica finale identica al tuo codice
      }
    }
